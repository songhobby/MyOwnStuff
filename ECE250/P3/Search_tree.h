		...
		if ( left_tree == nullptr ) {
			...
			if ( left_tree->insert( obj, left_tree ) ) {
				if (left_tree->height()-right_tree->height() >=2) {
						// special case for left left
					if (left_tree->right_tree == nullptr) {
						old_to_this = to_this;
						old_left_tree = left_tree;
						old_left_tree_left_tree = left_tree->left_tree;

						to_this = old_left_tree;
						left_tree = old_left_tree_left_tree;
						right_tree = old_to_this;
						right_tree->left_tree = nullptr;

						previous_node = left_tree;
						left_tree->next_node = to_this;
					}
					//sepcial case for left right
					else if (left_tree->left_tree == nullptr) {
						old_to_this = to_this;
						old_left_tree = left_tree;
						old_left_tree_right_tree = left_tree->right_tree;

						to_this = old_left_tree_right_tree;
						left_tree = old_left_tree;
					...
					//left left
						...
						to_this = old_left_tree;
						right_tree = old_to_this;
						right_tree->left_tree = old_left_tree_right_tree;

						//Update previous_node and next_node
						// Traverse the left_tree to do the updating
						Update_link(containing_tree->root_node,obj);
					}
					//left right
					else if (left_tree->left_tree->height() < left_tree->right_tree->height()) {
						old_to_this = to_this;
						old_left_tree = left_tree;
						old_left_tree_right_tree = left_tree->right_tree;
						old_left_tree_right_tree_left_tree = left_tree->right_tree->left_tree;
						old_left_tree_right_tree_right_tree =
						left_tree->right_tree->right_tree;

						to_this = old_left_tree_right_tree;
						left_tree = old_left_tree;
						right_tree = old_to_this;
						left_tree->right_tree = old_left_tree_right_tree_left_tree;
						right_tree->left_tree = old_left_tree_right_tree_right_tree;
						//Update previous_node and next_node
						// Traverse the left_tree to do the updating
						Update_link(containing_tree->root_node,obj);
					}
...
typename Search_tree<Type>::Iterator &Search_tree<Type>::Iterator::operator++() {
	...
	if (current_node != containing_tree->sentinel_back) {
...
typename Search_tree<Type>::Iterator &Search_tree<Type>::Iterator::operator--() {
	...
	if (current_node != containing_tree->sentinel_front) {
